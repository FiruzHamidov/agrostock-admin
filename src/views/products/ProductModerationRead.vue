<template>
  <div class="app-container moderation-read">
    <div class="read-header">
      <div>
        <h2>Модерация товара #{{ item.id || '' }}</h2>
        <p>{{ getModerationLabel(item.status) }} | {{ item.createdAt | dateTime }}</p>
      </div>
      <div class="read-actions">
        <router-link
          :to="{ name: 'editProductModeration', params: { id: item.id } }"
          tag="button"
          class="el-button"
        >
          Изменить
        ...
        <router-link
          v-if="item.productId"
          :to="{ name: 'readProducts', params: { id: item.productId } }"
          tag="button"
        ...
        <el-button @click="$router.push({ name: 'ProductModeration' })">Назад</el-button>
      </div>
    </div>
    ...
    <el-card v-loading="isLoading" class="read-card" shadow="never">
      <el-row :gutter="24">
        ...
        <el-col :xs="24" :sm="12" :md="8">
          <div class="read-field">
            <span>Товар</span>
            <strong>{{ getProductLabel(item.product, item.productId) }}</strong>
          </div>
        </el-col>
        <el-col :xs="24" :sm="12" :md="8">
          <div class="read-field">
            <span>Компания</span>
            <strong>{{ getCompanyLabel(item.company, item.companyId) }}</strong>
          </div>
        </el-col>
        <el-col :xs="24" :sm="12" :md="8">
          <div class="read-field">
...
export default {
  ...
  methods: {
    async fetchData() {
      this.isLoading = true
      try {
        this.item = await this.$apiClient.service('product-moderation').get(this.$route.params.id)
      } catch (err) {
    ...
    async moderate(status) {
      ...
      try {
        await this.$apiClient.service('product-moderation').patch(this.$route.params.id, { status, comment })

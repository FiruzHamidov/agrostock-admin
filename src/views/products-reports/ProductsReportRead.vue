<template>
  <div class="app-container report-read">
    <div class="read-header">
      <div>
        <h2>Жалоба на товар #{{ report.id || '' }}</h2>
        <p>{{ getTypeLabel(report.type) }} | {{ report.createdAt | dateTime }}</p>
      </div>
      <div class="read-actions">
        <router-link
          :to="{ name: 'editProductsReport', params: { id: report.id } }"
          tag="button"
          class="el-button el-button--primary"
        >
          Изменить
        </router-link>
        <el-button @click="$router.push({ name: 'ProductsReports' })">Назад</el-button>
      </div>
    </div>
    ...
    <el-card v-loading="isLoading" class="read-card" shadow="never">
      <el-row :gutter="24">
        ...
        <el-col :xs="24" :sm="12" :md="8">
          <div class="read-field">
            <span>Товар</span>
            <strong>{{ getProductLabel(report.product, report.productId) }}</strong>
          </div>
        </el-col>
        <el-col :xs="24" :sm="12" :md="8">
          <div class="read-field">
...
export default {
  ...
  methods: {
    async fetchData() {
      this.isLoading = true
      try {
        this.report = await this.$apiClient.service('products-reports').get(this.$route.params.id)
      } catch (err) {

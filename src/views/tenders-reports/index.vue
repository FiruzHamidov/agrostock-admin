  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" label="Название">
        <template slot-scope="scope">
          <template v-if="scope.row.tender">
            <router-link
              :to="{ name: 'editTender', params: { id: scope.row.tenderId } }"
              target="_blank"
            >
              {{ scope.row.tender.name }} - #{{ scope.row.tenderId }}
            </router-link>
          </template>
...
export default {
  ...
  methods: {
    async fetchData() {
      const tendersReportsService = this.$apiClient.service('tenders-reports')
